...
def smatch_score(file1, file2):
    ...
    for file_path in [file1, file2]:
        if not os.path.exists(file_path):
            print("Given file", file_path, "does not exist", file=ERROR_LOG)
            exit(1)
        file_handle.append(open(file_path))
        # use opened files
    #print(file_handle)
    file_handle = tuple(file_handle)
    for (precision, recall, best_f_score) in score_amr_pairs(file_handle[0], file_handle[1],
                                                             False,
                                                             False,
                                                             False):
        #print("F-score: " + floatdisplay % best_f_score)
